...
This is despite the fact that `reducer` does have a return type. The reason the return type isn't sufficient is because it doesn't flow through to the objects returned by `UpdateFoo` and `UpdateBar`.
